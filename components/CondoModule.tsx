...
import { Condominium, UserRole, User } from '../types';
import CondoModal from './CondoModal';
import ConfirmModal from './ConfirmModal';

interface CondoModuleProps {
  condos: Condominium[];
  user: User;
  onAddCondo: (condo: Omit<Condominium, 'id'>) => Promise<void>;
  onEditCondo: (id: string, condo: Partial<Condominium>) => Promise<void>;
  onDeleteCondo: (id: string) => Promise<void>;
}

const CondoModule: React.FC<CondoModuleProps> = ({ condos, user, onAddCondo, onEditCondo, onDeleteCondo }) => {
  ...
  const handleSave = async (condoData: Omit<Condominium, 'id'>) => {
    if (editingCondo) {
        await onEditCondo(editingCondo.id, condoData);
    } else {
        await onAddCondo(condoData);
    }
  };

  const confirmDelete = async () => {
    if (deleteId) {
        await onDeleteCondo(deleteId);
        setDeleteId(null);
    }
  };
  ...
  return (
    <div className="max-w-full mx-auto space-y-6">
      ...
      <ConfirmModal
        ...
        description="Tem certeza que deseja excluir este condomínio? Todos os dados associados (usuários, reservas) podem ser perdidos."
        confirmText="Excluir"
        isDestructive={true}
      />
    </div>
